...
import { searchLocal, searchOnline, lookupBarcode, defaultServing, nutrientsFor, logSavedMeal, recentFoods, frequentFoods, favoriteFoods } from "@/lib/foodRepo";
import { Sheet, Chip, useToast, EmptyState } from "@/components/ui";
...
export default function AddFoodSheet({ open, onClose, date, mealId, meals, nutrientTargets, macroTargets }: {
  ...
  useEffect(() => { if (open) { setMeal(mealId); setQ(""); setLocal([]); setOnline([]); setErrors([]); setTimeout(() => inputRef.current?.focus(), 250); } }, [open, mealId]);
  ...
  return (
    <>
      <Sheet open={open} onClose={onClose} size="lg" noPad title={<span>Add to <span className="text-accent">{mealName}</span></span>}>
            ...
            <>
              {tab === "meals" ? (
                ...
                (() => {
                  ...
                  return <div className="divide-y divide-line">{list.map((f) => <FoodRow key={f.id} food={f} onClick={() => setDetail(f)} />)}</div>;
                })()
              )}
            </>
          )}
